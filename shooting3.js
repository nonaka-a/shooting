...
const stage4IntroVideo = document.getElementById('stage4IntroVideo');
...
const HEART_ITEM_SPAWN_INTERVAL = 10000;
const POWERUP_SPAWN_INTERVAL = 15000;
const ENEMY_BULLET_SPEED = 3.5;
const ENEMY_FIRE_RATE = 6000;
const BOSS_BULLET_SPEED = 4;
const BOSS_FIRE_RATE = 5000;
const BOSS_APPEAR_SCORE = 8000;
const STAGE3_BOSS_HP = 3000;
...
const powerups = [];
const enemyBullets = [];
const bossExplosionParticles = [];
const boss = {};
let isBossActive = false;
let hasBossAppeared = false;
const titleParticles = [];

const background = { y1: 0, y2: -canvas.height, speed: 1.5 };
const keys = {};
...
const resources = {
    images: {
        background: { src: 'images/background/background3.png', asset: new Image() },
        ...
        bossStage3: { src: 'images/boss/boss_stage3.png', asset: new Image() },
        bossStage3Damaged: { src: 'images/boss/boss_stage3_damaged.png', asset: new Image() },
    },
    sounds: {
        playBGM: { src: 'sounds/stage3_play.mp3', asset: new Audio() },
        bossBGM: { src: 'sounds/stage3_boss.mp3', asset: new Audio() },
        gameClear: { src: 'sounds/gameclear.mp3', asset: new Audio() },
        finalClear: { src: 'sounds/game_ending.mp3', asset: new Audio() },
...
// --- 初期化・開始処理 ---
function resetGame() { const playerImage = resources.images.player1.asset; Object.assign(player, { width: playerImage.width * 0.2, height: playerImage.height * 0.2, x: canvas.width / 2 - (playerImage.width * 0.2) / 2, y: canvas.height - (playerImage.height * 0.2) - 20, speed: PLAYER_SPEED, baseSpeed: PLAYER_SPEED, dx: 0, dy: 0, fireRate: PLAYER_FIRE_RATE, lastBulletTime: 0, lives: 5, shotLevel: 1, speedLevel: 1, isDamaged: false, damageTimer: 0, animationFrame: 0, animationTimer: 0, animationDirection: 1, }); player.speed = player.baseSpeed + (player.speedLevel * 0.5); score = 0; bullets.length = 0; enemies.length = 0; hearts.length = 0; powerups.length = 0; enemyBullets.length = 0; Object.assign(boss, { x: canvas.width / 2 - 125, y: -250, width: 250, height: 250, baseWidth: 250, baseHeight: 250, hp: STAGE3_BOSS_HP, maxHp: STAGE3_BOSS_HP, speed: 1, dx: 1, isDefeated: false, defeatAlpha: 1.0, defeatScale: 1.0, lastBossBulletTime: 0, isDamagedState: false, currentScale: 1.0, scaleDirection: 1, targetY: 50, dy: 0, state: 'entering', dashSpeed: 12, returnSpeed: 1, lastDashTime: 0, dashInterval: 5333, aimTimer: 0, dashTargetX: 0, }); isBossActive = false; hasBossAppeared = false; bossExplosionParticles.length = 0; background.y1 = 0; background.y2 = -canvas.height; lastTime = 0; }
...
function updatePlayingState(deltaTime, currentTime) { updateBackground(); updatePlayer(currentTime); updateEntities(deltaTime); updateEnemyScaling(deltaTime); updateEnemyBullets(deltaTime); if (score >= BOSS_APPEAR_SCORE && !hasBossAppeared) { gameState = 'bossWarning'; bossWarningTimer = 8000; itemsDroppedDuringWarning = 0; enemyBullets.length = 0; fadeOutBGM(currentBGM, 1000); } if (isBossActive) { updateBoss(deltaTime, currentTime); } else { spawnEntities(currentTime); fireEnemyBullets(currentTime); } spawnItems(currentTime); checkCollisions(); }
...
function updateBoss(deltaTime, currentTime) {
    ...
    switch (boss.state) {
        case 'entering': boss.y += boss.dy; if (boss.y >= boss.targetY) { boss.y = boss.targetY; boss.dy = 0; boss.state = 'idle'; boss.lastDashTime = currentTime; } break;
        case 'idle': boss.x += boss.speed * boss.dx; if (boss.x + boss.width > canvas.width || boss.x < 0) { boss.dx *= -1; } if (currentTime - boss.lastBossBulletTime > BOSS_FIRE_RATE) { fireBossBullets(); boss.lastBossBulletTime = currentTime; } if (currentTime - boss.lastDashTime > boss.dashInterval) { boss.state = 'aiming'; boss.aimTimer = 1000; boss.dashTargetX = player.x + player.width / 2 - boss.width / 2; } break;
        case 'aiming': boss.aimTimer -= deltaTime; if (boss.aimTimer <= 0) { boss.state = 'dashing'; } break;
        case 'dashing': if (boss.x < boss.dashTargetX) boss.x = Math.min(boss.x + 5, boss.dashTargetX); if (boss.x > boss.dashTargetX) boss.x = Math.max(boss.x - 5, boss.dashTargetX); boss.y += boss.dashSpeed; if (boss.y > canvas.height) { boss.state = 'returning'; boss.y = -boss.height; boss.x = Math.random() * (canvas.width - boss.width); } break;
        case 'returning': boss.y += boss.returnSpeed; if (boss.y >= boss.targetY) { boss.y = boss.targetY; boss.state = 'idle'; boss.lastDashTime = currentTime; } break;
    }
}
function fireBossBullets() { const targetX = player.x + player.width / 2; const targetY = player.y + player.height / 2; const bulletX1 = boss.x + boss.width / 4; const bulletX2 = boss.x + boss.width * 3 / 4; const bulletY = boss.y + boss.height - 40; let angle1 = Math.atan2(targetY - bulletY, targetX - bulletX1); enemyBullets.push({ x: bulletX1 - 15, y: bulletY, width: 30, height: 30, speed: BOSS_BULLET_SPEED, dx: Math.cos(angle1) * BOSS_BULLET_SPEED, dy: Math.sin(angle1) * BOSS_BULLET_SPEED, frame: 0, animationTimer: 0, }); let angle2 = Math.atan2(targetY - bulletY, targetX - bulletX2); enemyBullets.push({ x: bulletX2 - 15, y: bulletY, width: 30, height: 30, speed: BOSS_BULLET_SPEED, dx: Math.cos(angle2) * BOSS_BULLET_SPEED, dy: Math.sin(angle2) * BOSS_BULLET_SPEED, frame: 0, animationTimer: 0, }); }
...
function spawnEntities(currentTime) { if (currentTime - gameStartTime < 2000) return; if (currentTime - lastEnemySpawnTime > ENEMY_SPAWN_INTERVAL) { if (Math.random() < 0.3) { const machoBaseWidth = 90; const machoBaseHeight = 90; enemies.push({ type: 'macho', x: Math.random() * (canvas.width - machoBaseWidth), y: -machoBaseHeight, width: machoBaseWidth, height: machoBaseHeight, hp: 5, maxHp: 5, speed: 1.5, dashSpeed: 10, returnSpeed: 2, state: 'descending', initialY: Math.random() * 100 + 40, aimTimer: 0, lastEnemyBulletTime: 0, canShoot: true, damageTimer: 0, }); } else { const enemyBaseWidth = 50; const enemyBaseHeight = 50; enemies.push({ type: 'normal', x: Math.random() * (canvas.width - enemyBaseWidth), y: -enemyBaseHeight, width: enemyBaseWidth, height: enemyBaseHeight, baseWidth: enemyBaseWidth, baseHeight: enemyBaseHeight, currentScale: 1.0, scaleDirection: Math.random() < 0.5 ? 1 : -1, speed: Math.random() * 2 + 1.5, lastEnemyBulletTime: 0, canShoot: Math.random() < 0.6, }); } lastEnemySpawnTime = currentTime; } }
...
function checkCollisions() {
    ...
    if (isBossActive && isColliding(player, boss)) {
...
function drawPlayingScreen(deltaTime) { const bgImage = resources.images.background.asset; if (bgImage && bgImage.complete && bgImage.naturalWidth > 0) { ctx.drawImage(bgImage, 0, Math.floor(background.y1), canvas.width, canvas.height); ctx.drawImage(bgImage, 0, Math.floor(background.y2), canvas.width, canvas.height); } drawEnemies(); drawItems(); drawBullets(deltaTime); drawEnemyBullets(); if (isBossActive || boss.isDefeated) { drawBoss(); } drawPlayer(); drawBossExplosionParticles(); drawUI(); }
...
function drawBoss() {
    ...
    const bossImg = resources.images.bossStage3.asset.complete && resources.images.bossStage3.asset.naturalWidth > 0 ? resources.images.bossStage3.asset : null;
    const bossDmgImg = resources.images.bossStage3Damaged.asset.complete && resources.images.bossStage3Damaged.asset.naturalWidth > 0 ? resources.images.bossStage3Damaged.asset : bossImg;
    ctx.save();
    if (boss.isDefeated) {
        ctx.globalAlpha = boss.defeatAlpha;
        if (bossDmgImg) {
            const scaledWidth = boss.baseWidth * boss.defeatScale;
            const scaledHeight = boss.baseHeight * boss.defeatScale;
            const drawX = boss.x + (boss.width / 2) - (scaledWidth / 2);
            const drawY = boss.y + (boss.height / 2) - (scaledHeight / 2);
            ctx.drawImage(bossDmgImg, drawX, drawY, scaledWidth, scaledHeight);
        }
    } else {
        const shadowImage = resources.images.playerShadow.asset;
        if (shadowImage && shadowImage.complete) {
            const shadowWidth = boss.width * 0.8;
            const shadowHeight = boss.height * 0.2;
            const shadowX = boss.x + (boss.width - shadowWidth) / 2;
            const shadowY = boss.y + boss.height - shadowHeight / 2 - 10;
            ctx.drawImage(shadowImage, shadowX, shadowY, shadowWidth, shadowHeight);
        }
        
        const bossImage = boss.isDamagedState ? bossDmgImg : bossImg;
        const finalWidth = boss.baseWidth;
        const finalHeight = boss.baseHeight;
        const drawX = boss.x;
        const drawY = boss.y;
...
function drawReadyScreen() {
    ...
    ctx.fillText('STAGE 3', canvas.width / 2, canvas.height / 2 - 40);
    ctx.font = '30px "M PLUS Rounded 1c"';
    if (Math.floor(Date.now() / 500) % 2 === 0) {
        ctx.fillText('クリックしてスタート', canvas.width / 2, canvas.height / 2 + 30);
    }
}
...
function drawStageClearScreen() {
    ...
    ctx.font = '48px "Yusei Magic"';
    ctx.textAlign = 'center';
    ctx.fillText('ステージ3 クリア！', canvas.width / 2, canvas.height / 2 - 80);
    ctx.font = '30px "M PLUS Rounded 1c"';
...
if (nextStageButton) {
    nextStageButton.addEventListener('click', () => {
        if (gameState === 'stageClear') {
            ...
            if (videoContainer) {
                stage4IntroVideo.muted = !isSoundOn;
                videoContainer.style.display = 'block';
                stage4IntroVideo.load();
                stage4IntroVideo.play();
            } else {
                window.location.href = 'shooting4.html';
            }
        }
    });
}
...
loadResources(() => {
    ...
    if (stage4IntroVideo) {
        stage4IntroVideo.addEventListener('ended', goToNextStage);
    }
    if (skipButton) {
        skipButton.addEventListener('click', () => {
            if (videoContainer && videoContainer.style.display === 'block') {
                stage4IntroVideo.pause();
                goToNextStage();
            }
        });
    }
